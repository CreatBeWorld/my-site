...
// 注意当get方法带参数时，需要使用正则表达式来匹配请求地址
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", (options) => {
  const { categoryid, limit, page } = queryString.parse(
    options.url.substring(options.url.indexOf("?") + 1)
  );
  const data = {
    "total|500-1000": 0,
    //注意当生成规则不是字面量时，需要使用属性表达式的语法来书写属性名和生成规则
    [`rows|${limit}`]: [
      {
        id: "@guid",
        title: "@ctitle(4,8)",
        ...
        "scanNumber|100-3000": 0,
        "commentNumber|100-500": 0,
        //从[]中的值二选一
        "thumb|1": ["@image(150x120,@color,#fff,@natural)"],
        createDate: "@date(T)",
      },
    ],
  }
...
Mock.mock(/^\/api\/comment(\?.+)?$/, "get", (options) => {
  const { blogid, limit, page } = queryString.parse(
    options.url.substring(options.url.indexOf("?") + 1)
  );
  return Mock.mock({
    code: 0,
